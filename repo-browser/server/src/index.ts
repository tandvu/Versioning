    ...
    try {
      ...
      if (repo.toLowerCase() === 'opt-soa') {
        ...
        if (hasNpmBuildScript) {
          ...
          let requiredPrivatePackages: string[] = [];
          ...
          if (fs.existsSync(packageJsonPath)) {
            try {
              ...
              requiredPrivatePackages = Object.keys(dependencies).filter(pkg =>
                pkg.startsWith('@gots/') || pkg.startsWith('@mtc2/')
              );
          ...
          const buildResult: { code: number; stdout: string; stderr: string } = await runStep(npmCmd, ['run', 'build'], { cwd: buildCwd, stdio: ['ignore', 'pipe', 'pipe'] }, 'npm run build');
